  ...
  <div class="dashboard-layout-wrapper">
    <div class="top-games-section">
      <div class="section-header">
        <h2 class="section-title">All Played Games</h2>
      </div>
      ...
      <div class="games-summary-bar" v-if="topGames && topGames.length > 0">
        <div class="summary-item">
          <span class="summary-label">Games Played</span>
          <span class="summary-value">{{ statsSummary.totalCount }}</span>
        </div>
        <div class="summary-divider"></div>
        <div class="summary-item">
          <span class="summary-label">Total Playtime</span>
          <span class="summary-value">{{ formatHours(statsSummary.totalHours) }}</span>
        </div>
      </div>
    </div>
  </div>
...
const statsSummary = computed(() => {
  const totalCount = topGames.value.length;
  const totalHours = topGames.value.reduce((acc, game) => acc + (game.totalHoursPlayed || 0), 0);
...
const navigateToGame = (appid: number | undefined) => {
  ...
  router.push(`/user/${currentUserId}/game/${appid}`);
};

onMounted(async () => {
  try {
    const userId = route.params.steamId || route.params.userId; 
    
    if (!userId) {
      console.error('No ID parameter extracted from the active route context.');
      return;
    }
    ...
    if (data.success && Array.isArray(data.games)) {
      topGames.value = data.games;
    }
  } catch (error) {
    console.error('Failed to resolve client-side data payload array:', error);
  }
});
</script>
